...
<template>
  <div>
    ...
    <div v-else-if="!fetchError" class="bg-white/70 backdrop-blur-2xl rounded-2xl border border-cupertino-gray-100/60 shadow-sm overflow-hidden">
            ...
            <tr v-for="p in filtered" :key="p.ID" class="border-b border-cupertino-gray-100/40 hover:bg-cupertino-gray-50/50 transition-colors">
              <td class="px-5 py-3">
                <router-link :to="`/patients/${p.ID}`" class="flex items-center gap-3">
                  ...
                  <span class="text-sm font-medium text-cupertino-gray-900">{{ p.firstName }} {{ p.lastName }}</span>
                </router-link>
              </td>
